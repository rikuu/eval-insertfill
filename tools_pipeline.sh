...
if [ ! -f tmp.filled.all.normal ]; then
  ...
  tail -n19 gap2seq.stderr | head -n1 | cut -c53- >> times
fi
...
if [ ! -f tmp.filled.all.filter ]; then
  ...
  tail -n19 gap2seq.filter.stderr | head -n1 | cut -c53- >> times
fi
...
if [ ! -f tmp.filled.pindel ]; then
  ...
  tail -n19 pindel.stderr | head -n1 | cut -c53- >> times
...
if [ ! -f tmp.filled.mtg ]; then
  ...
  tail -n19 mindthegap.stderr | head -n1 | cut -c53- >> times
...
if [ ! -f tmp.filled.gapfiller ]; then
  ...
  tail -n19 gapfiller.stderr | head -n1 | cut -c53- >> times
...
if [ ! -f tmp.filled.gapcloser ]; then
  ...
  tail -n19 gapcloser.stderr | head -n1 | cut -c53- >> times
fi
...
if [ ! -f tmp.filled.sealer ]; then
  ...
  tail -n19 sealer.stderr | head -n1 | cut -c53- >> times
